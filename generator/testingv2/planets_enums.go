...
const _planet_name = "MercuryVenusEarthMarsJupiterSaturnUranusNeptune"

var _planet_index = [...]uint8{0, 7, 12, 17, 21, 28, 34, 40, 47}

// String returns the string representation of the Planet value.
func (p Planet) String() string {
    // Extract the underlying enum value
    v := p.planet
    // Adjust for non-zero start index
    i := int(v) - 1
    
    // Check if the value is in range
    if i < 0 || i >= len(_planet_index)-1 {
        return fmt.Sprintf("planet(%d)", v)
    }
    
    // Return the string from the slice
