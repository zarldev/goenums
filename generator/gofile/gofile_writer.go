...
// lowercaseAliases returns the parse-map keys lower-cased and de-duplicated so
// the generated lookup, which lower-cases its input, matches any input casing.
func lowercaseAliases(aliases []string) []string {
	lowered := make([]string, 0, len(aliases))
	for _, a := range aliases {
		lwr := strings.ToLower(a)
		if slices.Contains(lowered, lwr) {
			continue
		}
		lowered = append(lowered, lwr)
	}
	return lowered
}
