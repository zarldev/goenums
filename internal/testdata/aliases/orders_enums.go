...
func (p Order) MarshalYAML() (any, error) {
	return p.String(), nil
}
...
func (p *Order) UnmarshalYAML(ctx context.Context, f func(any) error) error {
	select {
	case <-ctx.Done():
		return ctx.Err()
	default:
...
func (p Order) String() string {
	if str, ok := orderNamesMap[p]; ok {
		return str
	}
	return fmt.Sprintf("order(%d)", p.order)
}
