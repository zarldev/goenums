...
// MatchAlgorithm dispatches to the matcher method for the given enum value.
// The AlgorithmMatcher interface provides compile-time exhaustiveness: when enum
...
func MatchAlgorithm(en Algorithm, matcher AlgorithmMatcher) error {
	if matcher == nil {
		return fmt.Errorf("nil AlgorithmMatcher")
	}
	switch en {
	case Algorithms.AES256:
		matcher.AES256()
	case Algorithms.CHACHA20:
		matcher.ChaCha20()
	default:
		return fmt.Errorf("unhandled Algorithm: %v", en)
	}
	return nil
}

// MustMatchAlgorithm dispatches to the matcher method for the given enum value.
// It panics if matcher is nil or the enum value is not handled.
func MustMatchAlgorithm(en Algorithm, matcher AlgorithmMatcher) {
	if err := MatchAlgorithm(en, matcher); err != nil {
		panic(err)
	}
}
...
func (a *Algorithm) UnmarshalJSON(by []byte) error {
	by = bytes.Trim(bytes.Trim(by, "\""), "\"")
	newa, err := ParseAlgorithm(by)
	if err != nil {
		return err
	}
	*a = newa
...
func (a *Algorithm) UnmarshalText(by []byte) error {
	newa, err := ParseAlgorithm(by)
	if err != nil {
		return err
	}
...
// It returns an error if the byte slice does not contain a valid enum value.
func (a *Algorithm) UnmarshalBinary(by []byte) error {
	newa, err := ParseAlgorithm(by)
	if err != nil {
		return err
	}
...
func (a *Algorithm) UnmarshalYAML(by []byte) error {
	newa, err := ParseAlgorithm(by)
	if err != nil {
		return err
	}
