...
type DiscountTypeMatcher interface {
	Sale()
	Percentage()
	Amount()
	Giveaway()
...
// MatchDiscountType dispatches to the matcher method for the given enum value.
// The DiscountTypeMatcher interface provides compile-time exhaustiveness: when enum
...
func MatchDiscountType(en DiscountType, matcher DiscountTypeMatcher) error {
	if matcher == nil {
		return fmt.Errorf("nil DiscountTypeMatcher")
	}
	switch en {
	case DiscountTypes.SALE:
		matcher.Sale()
	...
	case DiscountTypes.GIVEAWAY:
		matcher.Giveaway()
	default:
		return fmt.Errorf("unhandled DiscountType: %v", en)
	}
	return nil
}

// MustMatchDiscountType dispatches to the matcher method for the given enum value.
// It panics if matcher is nil or the enum value is not handled.
func MustMatchDiscountType(en DiscountType, matcher DiscountTypeMatcher) {
	if err := MatchDiscountType(en, matcher); err != nil {
		panic(err)
	}
}
