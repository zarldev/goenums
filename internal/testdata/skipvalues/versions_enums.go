...
func (p Version) MarshalYAML() (any, error) {
	return p.String(), nil
}
...
func (p *Version) UnmarshalYAML(ctx context.Context, f func(any) error) error {
	select {
	case <-ctx.Done():
		return ctx.Err()
	default:
...
func (p Version) String() string {
	if str, ok := versionNamesMap[p]; ok {
		return str
	}
	return fmt.Sprintf("version(%d)", p.version)
}
