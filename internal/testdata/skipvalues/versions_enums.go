...
// MatchVersion dispatches to the matcher method for the given enum value.
// The VersionMatcher interface provides compile-time exhaustiveness: when enum
...
func MatchVersion(en Version, matcher VersionMatcher) error {
	if matcher == nil {
		return fmt.Errorf("nil VersionMatcher")
	}
	switch en {
	case Versions.V1:
		matcher.V1()
	...
	case Versions.V7:
		matcher.V7()
	default:
		return fmt.Errorf("unhandled Version: %v", en)
	}
	return nil
}

// MustMatchVersion dispatches to the matcher method for the given enum value.
// It panics if matcher is nil or the enum value is not handled.
func MustMatchVersion(en Version, matcher VersionMatcher) {
	if err := MatchVersion(en, matcher); err != nil {
		panic(err)
	}
}
...
func (v *Version) UnmarshalJSON(by []byte) error {
	by = bytes.Trim(bytes.Trim(by, "\""), "\"")
	newv, err := ParseVersion(by)
	if err != nil {
		return err
	}
	*v = newv
...
func (v *Version) UnmarshalText(by []byte) error {
	newv, err := ParseVersion(by)
	if err != nil {
		return err
	}
...
// It returns an error if the byte slice does not contain a valid enum value.
func (v *Version) UnmarshalBinary(by []byte) error {
	newv, err := ParseVersion(by)
	if err != nil {
		return err
	}
...
func (v *Version) UnmarshalYAML(by []byte) error {
	newv, err := ParseVersion(by)
	if err != nil {
		return err
	}
