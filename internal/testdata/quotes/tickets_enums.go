...
// MatchTicket dispatches to the matcher method for the given enum value.
// The TicketMatcher interface provides compile-time exhaustiveness: when enum
...
func MatchTicket(en Ticket, matcher TicketMatcher) error {
	if matcher == nil {
		return fmt.Errorf("nil TicketMatcher")
	}
	switch en {
	...
	case Tickets.PENDING:
		matcher.Pending()
	case Tickets.APPROVAL_PENDING:
		matcher.Approval_pending()
	case Tickets.APPROVAL_ACCEPTED:
		matcher.Approval_accepted()
	case Tickets.REJECTED:
		matcher.Rejected()
	case Tickets.COMPLETED:
		matcher.Completed()
	default:
		return fmt.Errorf("unhandled Ticket: %v", en)
	}
	return nil
}

// MustMatchTicket dispatches to the matcher method for the given enum value.
// It panics if matcher is nil or the enum value is not handled.
func MustMatchTicket(en Ticket, matcher TicketMatcher) {
	if err := MatchTicket(en, matcher); err != nil {
		panic(err)
	}
}
...
func (t *Ticket) UnmarshalJSON(by []byte) error {
	by = bytes.Trim(bytes.Trim(by, "\""), "\"")
	newt, err := ParseTicket(by)
	if err != nil {
		return err
	}
	*t = newt
...
func (t *Ticket) UnmarshalText(by []byte) error {
	newt, err := ParseTicket(by)
	if err != nil {
		return err
	}
...
// It returns an error if the byte slice does not contain a valid enum value.
func (t *Ticket) UnmarshalBinary(by []byte) error {
	newt, err := ParseTicket(by)
	if err != nil {
		return err
	}
...
func (t *Ticket) UnmarshalYAML(by []byte) error {
	newt, err := ParseTicket(by)
	if err != nil {
		return err
	}
