...
func (p Ticket) MarshalYAML() (any, error) {
	return p.String(), nil
}
...
func (p *Ticket) UnmarshalYAML(ctx context.Context, f func(any) error) error {
	select {
	case <-ctx.Done():
		return ctx.Err()
	default:
...
func (p Ticket) String() string {
	if str, ok := ticketNamesMap[p]; ok {
		return str
	}
	return fmt.Sprintf("ticket(%d)", p.ticket)
}
