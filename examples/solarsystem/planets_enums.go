...
// MatchPlanet dispatches to the matcher method for the given enum value.
// The PlanetMatcher interface provides compile-time exhaustiveness: when enum
...
func MatchPlanet(en Planet, matcher PlanetMatcher) error {
	if matcher == nil {
		return fmt.Errorf("nil PlanetMatcher")
	}
	switch en {
	...
	case Planets.NEPTUNE:
		matcher.Neptune()
	default:
		return fmt.Errorf("unhandled Planet: %v", en)
	}
	return nil
}

// MustMatchPlanet dispatches to the matcher method for the given enum value.
// It panics if matcher is nil or the enum value is not handled.
func MustMatchPlanet(en Planet, matcher PlanetMatcher) {
	if err := MatchPlanet(en, matcher); err != nil {
		panic(err)
	}
}
