...
func (p DiscountType) MarshalYAML() (any, error) {
	return p.String(), nil
}
...
func (p *DiscountType) UnmarshalYAML(ctx context.Context, f func(any) error) error {
	select {
	case <-ctx.Done():
		return ctx.Err()
	default:
...
func (p DiscountType) String() string {
	if str, ok := discounttypesNamesMap[p]; ok {
		return str
	}
	return fmt.Sprintf("discounttypes(%d)", p.discountTypes)
}
