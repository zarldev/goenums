...
// MarshalYAML implements the gopkg.in/yaml.v2::yaml.Marshaler interface for Planet.
// It returns the YAML string representation of the enum.
func (p Planet) MarshalYAML() (interface{}, error) {
	return p.String(), nil
}

// UnmarshalYAML implements the gopkg.in/yaml.v2::yaml.Unmarshaler interface for Planet.
// It supports unmarshaling from a YAML string representation of the enum.
func (p *Planet) UnmarshalYAML(unmarshal func(interface{}) error) error {
	var s string
	if err := unmarshal(&s); err != nil {
		return err
	}
	newp, err := ParsePlanet([]byte(s))
	if err != nil {
		return err
	}
	*p = newp
...
func (i planet) String() string {
	if i < 0 || i >= planet(len(planetsIdx)-1) {
